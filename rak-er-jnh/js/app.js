...
function roleLabel(r) {
  return r === "superadmin" ? "แอดมินใหญ่" : r === "admin" ? "แอดมินร่วม" : "สมาชิก";
}
...
async function viewHome(v) {
  ...
  if (!coworkers.length) { box.innerHTML = `<div class="empty">ยังไม่มีใครสแกนเข้าเวรนี้<br/>ไปที่แท็บ "เข้าเวร" เพื่อสแกนบาร์โค้ด</div>`; return; }
...
function openUserModal(u, isSuper) {
  ...
  const roleOptions = ["member", "admin"].concat(isSuper ? ["superadmin"] : [])
