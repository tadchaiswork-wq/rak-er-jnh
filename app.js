...
// ปัจจัยความพึงพอใจเวร (ใช้ได้ทั้งฝั่งพอใจและไม่พอใจ)
const SAT_FACTORS = ["บุคคล", "จำนวนผู้ป่วย", "ความยากของเคส", "การส่งเวร", "เพื่อนร่วมงาน", "อากาศ"];
...
/* ============================================================
   MY REPORTS — ย้อนดูรายงานที่ตนเองเขียน (แก้/ลบได้ภายใน 7 วัน)
   ============================================================ */
async function viewMyReports(v) {
  v.innerHTML = `
    <div class="card">
      <div class="section-title">🗂️ การรายงานของฉัน</div>
      <div class="sub">รายงานที่คุณเคยเขียน • แก้ไขรายละเอียด/ลบได้ภายใน 7 วันหลังบันทึก • การเพิ่มรายงานใหม่ยังทำได้เฉพาะภายใน 3 ชม.หลังลงเวร (แท็บ "รายงาน")</div>
      <div class="field" style="max-width:220px"><label>เลือกเดือน</label>
        <input type="month" id="mr-month" value="${ym(new Date())}" /></div>
      <div id="mr-body"><div class="empty">กำลังโหลด…</div></div>
    </div>`;
  const load = () => loadMyReports(el("mr-month").value);
  el("mr-month").onchange = load;
  load();
}

async function loadMyReports(month) {
  const body = el("mr-body");
  body.innerHTML = `<div class="empty">กำลังโหลด…</div>`;
  const snap = await db.collection("reports")
    .where("ym", "==", month).where("fromUid", "==", State.user.uid).get();
  const rows = snap.docs.map((d) => ({ id: d.id, ...d.data() }));
  rows.sort((a, b) => (b.shiftKey || "").localeCompare(a.shiftKey || ""));
  if (!rows.length) { body.innerHTML = `<div class="empty">เดือนนี้คุณยังไม่ได้เขียนรายงานใคร</div>`; return; }
  body.innerHTML = rows.map((r) => {
    const editable = reportEditable(r);
    return `<div class="person-row" style="align-items:flex-start">
      <div class="avatar" style="background:var(--pink)">${initials(r.toName)}</div>
      <div class="meta">
        <div class="n">${esc(r.toName)} <span class="s">• เวร${SHIFT_SHORT[(r.shiftKey || "").split("_")[1]] || ""} ${esc((r.shiftKey || "").slice(0, 10))}</span></div>
        <div class="s" style="color:var(--ink)">${esc(r.detail || "")}</div>
        <div class="s">${editable ? "แก้ไข/ลบได้" : "เกิน 7 วันแล้ว — ดูได้อย่างเดียว"}</div>
      </div>
      <div class="acts">
        ${editable ? `<button class="btn btn-sm btn-outline" data-edit="${r.id}">แก้ไข</button>
        <button class="btn btn-sm btn-danger" data-del="${r.id}">ลบ</button>` : ""}
      </div>
    </div>`;
  }).join("");
  const byId = {}; rows.forEach((r) => (byId[r.id] = r));
  body.querySelectorAll("button[data-edit]").forEach((b) => {
    b.onclick = () => openMyReportModal(byId[b.dataset.edit], month);
  });
  body.querySelectorAll("button[data-del]").forEach((b) => {
    b.onclick = () => deleteMyReport(byId[b.dataset.del], month);
  });
}

function reportEditable(r) {
  const t = r.createdAt && r.createdAt.toDate ? r.createdAt.toDate().getTime() : 0;
  if (!t) return true; // ไม่มี timestamp (ข้อมูลเก่า) — ให้แก้ได้
  return (Date.now() - t) <= EDIT_REPORT_MS;
}

function openMyReportModal(r, month) {
  showModal(`
    <h3>แก้ไขรายงาน: ${esc(r.toName)}</h3>
    <div class="sub">เวร${SHIFT_SHORT[(r.shiftKey || "").split("_")[1]] || ""} ${esc((r.shiftKey || "").slice(0, 10))}</div>
    <div class="field"><label>รายละเอียดที่รายงาน</label>
      <textarea id="mr-detail">${esc(r.detail || "")}</textarea></div>
    <div class="btn-row">
      <button class="btn btn-primary" id="mr-save">บันทึกการแก้ไข</button>
      <button class="btn btn-danger" id="mr-del">ลบรายงาน</button>
      <button class="btn btn-outline" id="mr-cancel">ยกเลิก</button>
    </div>`);
  el("mr-cancel").onclick = closeModal;
  el("mr-save").onclick = async () => {
    const detail = el("mr-detail").value.trim();
    if (!detail) return toast("กรุณากรอกรายละเอียด", "err");
    try {
      await db.collection("reports").doc(r.id).update({
        detail, updatedAt: firebase.firestore.FieldValue.serverTimestamp(),
      });
      closeModal(); toast("บันทึกการแก้ไขแล้ว", "ok"); loadMyReports(month);
    } catch (e) { toast("บันทึกไม่สำเร็จ: " + friendlyErr(e), "err"); }
  };
  el("mr-del").onclick = () => { closeModal(); deleteMyReport(r, month); };
}

async function deleteMyReport(r, month) {
  if (!confirm(`ลบรายงานของ ${r.toName}?`)) return;
  try {
    await db.collection("reports").doc(r.id).delete();
    toast("ลบรายงานแล้ว"); loadMyReports(month);
  } catch (e) { toast("ลบไม่สำเร็จ: " + friendlyErr(e), "err"); }
}
...
async function viewSatisfaction(v) {
  ...
  v.innerHTML = `
    <div class="card">
      ...
      <div class="sub">เลื่อนแถบให้คะแนนเอง (0–10) • กดเลือกปัจจัยที่ทำให้พอใจ/ไม่พอใจ • เขียนข้อเสนอแนะเพิ่มได้ • แก้ไขได้ภายใน 3 ชม.หลังลงเวร</div>
      <div class="field"><label>เลือกเวร</label>
...
async function loadSatisfaction(shiftKey) {
  ...
  const rating = existing && typeof existing.rating === "number" ? existing.rating : 5;
  const good = new Set(existing && existing.goodFactors ? existing.goodFactors : []);
  const bad = new Set(existing && existing.badFactors ? existing.badFactors : []);
  const chipsHTML = (side, set) => SAT_FACTORS.map((f) =>
    `<button type="button" class="chip chip-${side} ${set.has(f) ? "on" : ""}" data-side="${side}" data-f="${esc(f)}">${esc(f)}</button>`).join("");
  body.innerHTML = `
    <div class="field"><label>คะแนนความพึงพอใจ — เลื่อนแถบเอง (0 = น้อยที่สุด, 10 = มากที่สุด)</label>
      <div class="sat-slider">
        <input type="range" id="st-range" min="0" max="10" step="1" value="${rating}" />
        <div class="sat-scale"><span>0</span><span>5</span><span>10</span></div>
        <div id="st-val" class="sat-val"></div>
      </div></div>
    <div class="field"><label>👍 สิ่งที่ทำให้ <b>พอใจ</b> (กดเลือกได้หลายข้อ)</label>
      <div class="chip-row" id="st-good">${chipsHTML("good", good)}</div></div>
    <div class="field"><label>👎 สิ่งที่ทำให้ <b>ไม่พอใจ</b> (กดเลือกได้หลายข้อ)</label>
      <div class="chip-row" id="st-bad">${chipsHTML("bad", bad)}</div></div>
    <div class="field"><label>อื่นๆ / ข้อเสนอแนะ (ไม่บังคับ)</label>
  ...
  const range = el("st-range"), valLbl = el("st-val");
  const drawVal = () => { valLbl.textContent = `${range.value}/10`; };
  range.oninput = drawVal; drawVal();
  const wire = (boxId, set) => el(boxId).querySelectorAll("button[data-f]").forEach((b) => {
    b.onclick = () => {
      const f = b.dataset.f;
      if (set.has(f)) { set.delete(f); b.classList.remove("on"); }
      else { set.add(f); b.classList.add("on"); }
    };
  });
  wire("st-good", good); wire("st-bad", bad);
  el("st-save").onclick = async () => {
    try {
      await docRef.set({
        ...
        rating: +range.value, goodFactors: [...good], badFactors: [...bad],
        suggestion: el("st-sug").value.trim(),
        createdAt: existing ? existing.createdAt : firebase.firestore.FieldValue.serverTimestamp(),
        updatedAt: firebase.firestore.FieldValue.serverTimestamp(),
      }, { merge: true });
...
/* นับจำนวนปัจจัย (goodFactors/badFactors) จากรายการพึงพอใจ -> [[factor, count], ...] เรียงมาก→น้อย */
function tallyFactors(sats, field) {
  const c = {};
  sats.forEach((r) => (r[field] || []).forEach((f) => (c[f] = (c[f] || 0) + 1)));
  return Object.entries(c).sort((a, b) => b[1] - a[1]);
}
/* แสดงปัจจัยของรายการพึงพอใจหนึ่งรายการ */
function factorLine(r) {
  const g = (r.goodFactors || []); const b = (r.badFactors || []);
  if (!g.length && !b.length) return "";
  const parts = [];
  if (g.length) parts.push(`👍 ${g.map(esc).join(", ")}`);
  if (b.length) parts.push(`👎 ${b.map(esc).join(", ")}`);
  return `<div class="s">${parts.join(" &nbsp;•&nbsp; ")}</div>`;
}
...
async function loadDashboard(month, showEval) {
  ...
  const goodTally = tallyFactors(sats, "goodFactors");
  const badTally = tallyFactors(sats, "badFactors");
  ...
  const tallyHTML = (tally, side) => tally.length
    ? `<div class="chip-row" style="margin-bottom:10px">${tally.map(([f, n]) => `<span class="chip chip-${side} on">${esc(f)} · ${n}</span>`).join("")}</div>`
    : `<div class="sub" style="margin-bottom:10px">— ไม่มี —</div>`;
  const satTable = `
    ...
    <div class="sub" style="margin-bottom:6px">👍 ปัจจัยที่ทำให้พอใจ (เรียงตามจำนวน)</div>
    ${tallyHTML(goodTally, "good")}
    <div class="sub" style="margin-bottom:6px">👎 ปัจจัยที่ทำให้ไม่พอใจ (เรียงตามจำนวน)</div>
    ${tallyHTML(badTally, "bad")}
  ...
  body.querySelectorAll("button[data-late]").forEach((b) => {
    b.onclick = () => showLateDetailModal(b.dataset.name, month, lateAgg[b.dataset.late], showEval);
  });
...
function showLateDetailModal(name, month, agg, showEval) {
  ...
  showModal(`
    ...
    <div class="sub">เดือน ${month} • รวม ${agg ? agg.totalMin : 0} นาที${showEval ? " • แอดมินใหญ่แก้เวลาเข้าเวรได้" : ""}</div>
    ${recs.length ? recs.map((c) => `<div class="person-row" style="margin-bottom:8px;align-items:flex-start"><div class="meta">
  ...
  const reload = () => { closeModal(); loadDashboard(month, showEval); };
  document.querySelectorAll("button[data-lateedit]").forEach((b) => {
    b.onclick = () => openEditCheckinTimeModal(recs.find((c) => c.id === b.dataset.lateedit), reload);
  });
  document.querySelectorAll("button[data-latedel]").forEach((b) => {
    b.onclick = () => adminDeleteRecord("checkins", b.dataset.latedel, "บันทึกเข้าเวรนี้", reload);
  });
}

/* เวลาเข้าเวรจริงเป็นข้อความ HH:MM */
function checkinTimeStr(c) {
  const d = c.ts && c.ts.toDate ? c.ts.toDate() : (c.ts ? new Date(c.ts) : null);
  return d ? d.toLocaleTimeString("th-TH", { hour: "2-digit", minute: "2-digit" }) : "-";
}
/* เวลาเริ่มเวร (Date) จาก date + shift */
function shiftStartDate(dateStr, shift) {
  const [y, m, d] = dateStr.split("-").map(Number);
  const h = shift === "morning" ? 8 : shift === "afternoon" ? 16 : 0;
  return new Date(y, m - 1, d, h, 0, 0);
}

/* แอดมินใหญ่: แก้เวลาเข้าเวร -> คำนวณสายใหม่ */
function openEditCheckinTimeModal(c, onDone) {
  if (!c) return;
  const cur = c.ts && c.ts.toDate ? c.ts.toDate() : (c.ts ? new Date(c.ts) : shiftStartDate(c.date, c.shift));
  const hhmm = `${pad(cur.getHours())}:${pad(cur.getMinutes())}`;
  showModal(`
    <h3>แก้เวลาเข้าเวร</h3>
    <div class="sub">${esc(c.fullName || "")} • ${esc(c.date)} เวร${SHIFT_SHORT[c.shift] || c.shift} (เริ่ม ${c.shift === "morning" ? "08:00" : c.shift === "afternoon" ? "16:00" : "00:00"})</div>
    <div class="field"><label>เวลาเข้าเวรจริง (HH:MM)</label>
      <input type="time" id="ec-time" value="${hhmm}" /></div>
    <div class="sub" id="ec-preview"></div>
    <div class="btn-row" style="margin-top:8px">
      <button class="btn btn-primary" id="ec-save">บันทึกเวลา</button>
      <button class="btn btn-outline" id="ec-cancel">ยกเลิก</button>
    </div>`);
  const start = shiftStartDate(c.date, c.shift);
  const calc = (val) => {
    const [h, mi] = val.split(":").map(Number);
    const nd = new Date(start); nd.setHours(h, mi, 0, 0);
    const lm = Math.max(0, Math.floor((nd - start) / 60000));
    return { nd, lm, late: lm > LATE_MIN };
  };
  const preview = () => {
    const { lm, late } = calc(el("ec-time").value);
    el("ec-preview").textContent = late ? `→ จะบันทึกว่า สาย ${lm} นาที` : `→ จะบันทึกว่า ไม่สาย`;
  };
  el("ec-time").oninput = preview; preview();
  el("ec-cancel").onclick = closeModal;
  el("ec-save").onclick = async () => {
    const { nd, lm, late } = calc(el("ec-time").value);
    try {
      await db.collection("checkins").doc(c.id).update({
        ts: firebase.firestore.Timestamp.fromDate(nd),
        lateMinutes: lm, late,
        updatedAt: firebase.firestore.FieldValue.serverTimestamp(),
      });
      toast("แก้เวลาเข้าเวรแล้ว", "ok"); if (onDone) onDone();
    } catch (e) { toast("บันทึกไม่สำเร็จ: " + friendlyErr(e), "err"); }
  };
}
...
function showDetailModal(uid, name, month, reports, apps, showEval) {
  ...
  showModal(`
    ...
    ${rs.length ? rs.map((r) => `<div class="person-row" style="margin-bottom:8px;align-items:flex-start"><div class="meta">
        ...
        <button class="btn btn-sm btn-outline" data-repedit="${r.id}">แก้</button>
        <button class="btn btn-sm btn-danger" data-repdel="${r.id}">ลบ</button></div>` : ""}
      </div>`).join("") : '<div class="empty">ไม่มี</div>'}
    <h4 style="margin:12px 0 6px">คำชื่นชม (${as.length})</h4>
    ${as.length ? as.map((r) => `<div class="person-row" style="margin-bottom:8px;align-items:flex-start"><div class="meta">
      ...
      </div>${showEval ? `<div class="acts"><button class="btn btn-sm btn-danger" data-appdel="${r.id}">ลบ</button></div>` : ""}
      </div>`).join("") : '<div class="empty">ไม่มี</div>'}
    <div class="btn-row" style="margin-top:14px">
  ...
  // ---- แอดมินใหญ่: แก้ไข/ลบรายงานและคำชื่นชม ----
  const reload = () => { closeModal(); loadDashboard(month, showEval); };
  document.querySelectorAll("button[data-repedit]").forEach((b) => {
    b.onclick = () => {
      const r = rs.find((x) => x.id === b.dataset.repedit);
      openAdminEditModal("reports", r.id, "รายละเอียดรายงาน", r.detail, "detail", reload);
    };
  });
  document.querySelectorAll("button[data-repdel]").forEach((b) => {
    b.onclick = () => adminDeleteRecord("reports", b.dataset.repdel, "รายงานนี้", reload);
  });
  document.querySelectorAll("button[data-appdel]").forEach((b) => {
    b.onclick = () => adminDeleteRecord("appreciations", b.dataset.appdel, "คำชื่นชมนี้", reload);
  });
}

/* แอดมินใหญ่: แก้ไขฟิลด์ข้อความของเอกสาร */
function openAdminEditModal(coll, docId, label, value, field, onDone) {
  showModal(`
    <h3>แก้ไข${esc(label)}</h3>
    <div class="field"><label>${esc(label)}</label>
      <textarea id="ae-val">${esc(value || "")}</textarea></div>
    <div class="btn-row">
      <button class="btn btn-primary" id="ae-save">บันทึก</button>
      <button class="btn btn-outline" id="ae-cancel">ยกเลิก</button>
    </div>`);
  el("ae-cancel").onclick = closeModal;
  el("ae-save").onclick = async () => {
    try {
      await db.collection(coll).doc(docId).update({
        [field]: el("ae-val").value.trim(),
        updatedAt: firebase.firestore.FieldValue.serverTimestamp(),
      });
      toast("บันทึกแล้ว", "ok"); if (onDone) onDone();
    } catch (e) { toast("บันทึกไม่สำเร็จ: " + friendlyErr(e), "err"); }
  };
}

/* แอดมินใหญ่: ลบเอกสาร */
async function adminDeleteRecord(coll, docId, what, onDone) {
  if (!confirm(`ยืนยันลบ${what}?`)) return;
  try {
    await db.collection(coll).doc(docId).delete();
    toast("ลบแล้ว"); if (onDone) onDone();
  } catch (e) { toast("ลบไม่สำเร็จ: " + friendlyErr(e), "err"); }
}
...
function printSatisfactionReport(month, sats, avg) {
  ...
  const good = tallyFactors(sats, "goodFactors");
  const bad = tallyFactors(sats, "badFactors");
  const tallyTxt = (t) => t.length ? t.map(([f, n]) => `${f} (${n})`).join(", ") : "—";
  const body = `<div class="muted">รายงานความพึงพอใจในการอยู่เวร • เดือน ${esc(month)}</div>
    ...
    <p><b>👍 ปัจจัยที่พอใจ:</b> ${esc(tallyTxt(good))}</p>
    <p><b>👎 ปัจจัยที่ไม่พอใจ:</b> ${esc(tallyTxt(bad))}</p>
